...
namespace SFSE
{
	...
	void* Trampoline::do_create(std::size_t a_size, std::uintptr_t a_address)
	{
		...
		std::uintptr_t       min = a_address >= minRange
				  ? detail::roundup(a_address - minRange, granularity)
				  : 0;
		const std::uintptr_t max = a_address < (maxAddr - minRange)
			? detail::rounddown(a_address + minRange, granularity)
			: maxAddr;
		std::uintptr_t       addr;
		...
		do {
			...
			if (mbi.State == MEM_FREE) {
				...
				// if rounding didn't advance us into the next region and the region is
				// the required size
				if (addr < min && (min - addr) >= a_size) {
					...
					if (mem) {
						...
						log::warn(
							"VirtualAlloc failed with code: 0x{:08X}"sv,
							::GetLastError());
					}
				}
			}
