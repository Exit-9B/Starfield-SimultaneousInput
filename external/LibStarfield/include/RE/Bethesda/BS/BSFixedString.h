...
#include "RE/Bethesda/BS/BSStringPool.h"
	...
	namespace detail
	{
		...
		public:
			...
			BSFixedString(const T& a_string) //
				requires(
					std::convertible_to<const T&, std::basic_string_view<value_type>> &&
			...
			{
				const auto view = static_cast<std::basic_string_view<value_type>>(
					a_string);
				if (!view.empty()) {
			...
			{
				const auto leaf = [](const BSFixedString& a_elem)
				{
					return a_elem._data ? a_elem._data->leaf() : nullptr;
				};
